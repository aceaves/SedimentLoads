...
merged1$Value <- formatC(merged1$Value, digits = 0, format = "f")
merged1$Value <- as.character(merged1$Value)
...
lm_model <- lm(Conc ~ Value, data = merged1)

p <- ggplot(data = merged1, aes(x = Value, y = Conc)) +
  geom_point(size = 4, color = "blue") +    # Customize the points
  geom_smooth(method = "lm", se = TRUE, color = "red") +  # Add regression
  labs(title = "Sediment Scatter Plot with Best-Fit Regression Line",   # Add title
       x = "Value (FNU)",                 # X-axis label
       y = "Conc (mg/l)") +               # Y-axis label
...
=======
################################################################################
#Script to calculate turbidity and inturn SSC from turbidity
#Created 16/08/2023 by Ashton Eaves
...
gg <- ggplot(data = merged1, aes(x = Value, y = Conc)) +
  geom_point(size = 4, color = "blue") +    # Customize the points
  geom_smooth(method = "lm", se = TRUE, color = "red") +  # Add regression
  labs(title = "Sediment Scatter Plot with Best-Fit Regression Line",   # Add title
       x = "Value (FNU)",                 # X-axis label
